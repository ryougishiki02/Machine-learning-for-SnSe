...
def setup_experiment(args):
    ...
    args.experiment = re.sub(r'[<>:"/\\|?*]', '', args.experiment)
...
def save_checkpoint(args, step, model, optimizer=None, scheduler=None, score=None, mode="min"):
    ...
    # Initialize attributes if they do not exist
    if not hasattr(save_checkpoint, 'last_step'):
        save_checkpoint.last_step = -1
    if not hasattr(save_checkpoint, 'best_score'):
        save_checkpoint.best_score = float("inf") if mode == "min" else float("-inf")
    if not hasattr(save_checkpoint, 'best_step'):
        save_checkpoint.best_step = -1

    # Get the current last_step and best_score
