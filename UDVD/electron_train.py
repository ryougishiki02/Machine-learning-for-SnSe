...
#                 break
            ...
            # print(noisy_frame.shape, outputs.shape)
            # print(noisy_frame.min(),noisy_frame.max())
            # print(outputs.min(), outputs.max())
            #
            # # 裁剪 outputs 的最小值和最大值
            # outputs = outputs.clamp(min=0, max=255)
            ...
            #             print(f"Found non-finite gradient in parameter: {param}")
            ...
            # print(f"Noisy inputs shape: {noisy_inputs[:, (mid*cpf):((mid+1)*cpf), :, :].shape}")  # 添加这行
            # print(f"Noisy outputs shape: {outputs.shape}")  # 添加这行
            ...
            if writer is not None and global_step % args.log_interval == 0:
                ...
                if gradients.numel() > 0:  # 确保梯度不是空的
                    writer.add_histogram("gradients", gradients, global_step)
                else:
                    print("No gradients to write to histogram")
            ...
            if (batch_id+1) % 200 == 0:
                if args.loss == "loglike":
                    ...
                    logging.info(train_bar.print(dict(**train_meters, lr=optimizer.param_groups[0][
...
def get_args():
    ...
    parser.add_argument("--stride", default=128, type=int, help="stride for patch extraction")
